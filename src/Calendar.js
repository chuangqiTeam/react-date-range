...
class Calendar extends Component {
    ...
    changeMonth(direction, event) {
        ...
        const {link, linkCB, minDate, maxDate} = this.props;
        const {shownDate} = this.state;

        const newMonth = this.getShownDate().clone().add(direction, 'months');

        var compareMinMonth = minDate ? moment([minDate.year(), minDate.month()]) : moment([1970, 7]);
        var compareMaxMonth = maxDate ? moment([maxDate.year(), maxDate.month()]) : moment([2200, 1]);
        var compareNewMonth = moment([newMonth.year(), newMonth.month()]);
    ...
    _onSelectMonth(event) {
        const currentMonth = this.getShownDate().month();
    ...
    _onSelectYear(event) {
        ...
        var toBeMonth = moment([event.target.value, shownDate.month()]);
        var compareMinMonth = minDate ? moment([minDate.year(), minDate.month()]) : moment([1970, 7]);
        var compareMaxMonth = maxDate ? moment([maxDate.year(), maxDate.month()]) : moment([2200, 1]);
        ...
        var direction = toBeMonth.diff(moment([shownDate.year(), shownDate.month()]), 'months');
    ...
    renderMonthAndYear(classes) {
        ...
        var yearArr = [];
        var minYear = minDate ? minDate.year() : moment().subtract(1, 'years').year();
        var maxYear = maxDate ? maxDate.year() : moment().year();
